...
function updateResultChart(result_emotion, result_gender) {
    ...
    if(flag==0) //尚未結帳
    {
        ...
        //if(s_price>25) {s_price=25}
      price=Math.floor(s_price/10)*10
      kprice=Math.floor(price/5)-10
      if(kprice<5){kprice=5}
      if(kprice>25){kprice=25}
       // s_price = s_price / 10
      console.log(kprice)
      //  s_price=Math.floor(s_price)
        $("#price").text(Math.floor(kprice))
    }
